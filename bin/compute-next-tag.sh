...
#
# Tags look like `v<Prometheus SSH exporter version>-<release>`:
#
# - if defaults/main.yml points at a Prometheus SSH exporter version that
#   has never been released, the release counter restarts at 0 (`v1.6.0-0`)
...
version="$(sed -nE 's|^prometheus_ssh_exporter_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Prometheus SSH exporter version from $defaults_path"
	exit 1
fi

# The version values carry a leading `v` (e.g. `v1.6.0`), and so do the tags
# (`v1.6.0-0`). Stripping any `v` before prepending one keeps this correct
